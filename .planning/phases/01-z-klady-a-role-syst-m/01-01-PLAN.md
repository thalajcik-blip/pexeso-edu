...
files_modified:
  ...
  - src/admin/components/UsersManager.tsx
  - src/admin/components/TeacherRequestsManager.tsx
...
must_haves:
  truths:
    - Admin app reads roles from profiles.roles[] without querying user_roles table
    - useAuth.ts fetchRole() returns correct role from profiles
    - get_users_with_roles RPC does not reference user_roles table
    - user_roles table is dropped
  artifacts:
    - supabase/migrations/ with drop_user_roles migration
  key_links:
    - useAuth.ts → profiles.roles[]
    - TeacherRequestsManager → profiles.roles[] (not user_roles)
---
# Plan: Role Migration
