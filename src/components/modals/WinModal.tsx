  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.88)' }}>
      <div className="pop-in rounded-2xl p-10 text-center"
        ...
        <div className="text-2xl font-bold mb-1" style={{ color: '#f9d74e' }}>Konec hry!</div>
        <div className="text-xs uppercase tracking-widest mb-6" style={{ color: 'rgba(255,255,255,0.5)' }}>Výsledky</div>

        {winners.length > 1
          ? <div className="text-lg mb-4" style={{ color: '#f9d74e' }}>🤝 Remíza!</div>
          : <div className="text-lg mb-4" style={{ color: '#f9d74e' }}>🏆 Vítěz: <strong>{winners[0].name}</strong></div>
        }
          ...
          {sorted.map((p, i) => (
            <div key={i}>
              <div className="text-base">
                {MEDALS[i]} <span style={{ color: p.color }}>{p.name}</span>: <strong>{p.score} bodů</strong>
              </div>
              <div className="text-xs pl-6 mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>
                🃏 {pluralize(p.pairs, 'pár', 'páry', 'párů')} &nbsp;+&nbsp; 🧠 {pluralize(p.quizzes, 'kvíz', 'kvízy', 'kvízů')}
              </div>
            </div>
          ))}
        </div>
